 ...
 * Android equivalent of iOS ElEspinarParser.
 */
class ElEspinarParser : ColumnBasedPDFParser(), PDFParsingStrategy {
    ...
    override fun parseSchedules(pdf: PDDocument): List<PharmacySchedule> {
        ...
        try {
            ...
            for (pageIndex in 0 until pdf.numberOfPages) {
                ...
                DebugConfig.debugPrint("📄 Processing El Espinar page ${pageIndex + 1} of ${pdf.numberOfPages}")
                
                // El Espinar typically uses a 2-column layout (Date, Pharmacy)
                ...
                DebugConfig.debugPrint("📄 El Espinar data - parsedDates: ${parsedDates.size}, pharmacies: ${pharmacies.size}")
            ...
            DebugConfig.debugPrint("✅ Successfully parsed ${allSchedules.size} schedules for El Espinar")
            return allSchedules.sortedWith { first, second ->
                compareSchedulesByDate(first, second)
            }
            
        } catch (e: Exception) {
            DebugConfig.debugPrint("❌ Error parsing El Espinar PDF: ${e.message}")
            e.printStackTrace()
            return emptyList()
        }
    }
