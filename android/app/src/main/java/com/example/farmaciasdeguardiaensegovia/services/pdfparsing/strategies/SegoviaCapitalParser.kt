...
class SegoviaCapitalParser : ColumnBasedPDFParser(), PDFParsingStrategy {
    ...
    override fun parseSchedules(pdfFile: File): Map<DutyLocation, List<PharmacySchedule>> {
        ...
        return try {
            ...
            mapOf(DutyLocation.fromRegion(Region.segoviaCapital) to allSchedules)
        } catch (e: Exception) {
        ...
        val maybeSchedule = if (dutyDate != null) {
            when {
                dayName != null && dayAddress != null && dayPhone != null &&
                    ...
                    PharmacySchedule(
                        dutyDate, mapOf(
                            DutyTimeSpan.CapitalDay to listOf(dayTimePharmacy),
                            DutyTimeSpan.CapitalNight to listOf(nightTimePharmacy)
                        )
                    )
                }
