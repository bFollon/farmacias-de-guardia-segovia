    ...
    override fun parseSchedules(pdf: PDDocument): List<PharmacySchedule> {
        ...
        try {
            ...
            for (pageIndex in 0 until pdf.numberOfPages) {
                ...
                DebugConfig.debugPrint("📄 Processing rural page ${pageIndex + 1} of ${pdf.numberOfPages}")
                
                // Extract column text from the page (rural areas typically use 2 columns)
                ...
                DebugConfig.debugPrint("📄 Rural data - parsedDates: ${parsedDates.size}, pharmacies: ${pharmacies.size}")
                
                // Create schedules for valid dates (rural areas typically have full-day shifts)
            ...
            DebugConfig.debugPrint("✅ Successfully parsed ${allSchedules.size} schedules for Segovia Rural")
            return allSchedules.sortedWith { first, second ->
                compareSchedulesByDate(first, second)
            }
            
        } catch (e: Exception) {
            DebugConfig.debugPrint("❌ Error parsing Segovia Rural PDF: ${e.message}")
            e.printStackTrace()
            return emptyList()
        }
    }
