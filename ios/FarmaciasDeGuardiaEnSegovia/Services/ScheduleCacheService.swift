...
class ScheduleCacheService {
    ...
    private init() {
        ...
        if !fileManager.fileExists(atPath: cacheDirectory.path) {
            ...
            DebugConfig.debugPrint("📂 Created schedule cache directory: \(cacheDirectory.path)")
        }
    }
    ...
    func isCacheValid(for region: Region) -> Bool {
        ...
        do {
            ...
            DebugConfig.debugPrint("❌ Error checking cache validity for \(region.name): \(error)")
            return false
        }
    }
    ...
    func loadCachedSchedules(for region: Region) -> [PharmacySchedule]? {
        ...
        do {
            ...
            DebugConfig.debugPrint("⚡ Loaded \(cachedData.schedules.count) cached schedules for \(region.name) in \(Int(loadTime))ms")
            return cachedData.schedules
            
        } catch {
            DebugConfig.debugPrint("❌ Error loading cached schedules for \(region.name): \(error)")
    ...
    func saveSchedulesToCache(for region: Region, schedules: [PharmacySchedule]) {
        do {
            ...
            let cacheFile = getCacheFile(for: region)
            let data = try json.encode(cachedData)
            try data.write(to: cacheFile)
            
            // Save metadata
            guard let pdfURL = PDFCacheManager.shared.cachedFileURL(for: region) else {
                DebugConfig.debugPrint("⚠️ No PDF file found for \(region.name), using current timestamp")
                return
            }
            ...
            let metadataFile = getMetadataFile(for: region)
            let metadataData = try json.encode(metadata)
            ...
            DebugConfig.debugPrint("💾 Cached \(schedules.count) schedules for \(region.name) in \(Int(saveTime))ms (\(cacheSize)KB)")
            
        } catch {
            DebugConfig.debugPrint("❌ Error saving schedules to cache for \(region.name): \(error)")
        }
    }
    ...
    func clearAllCache() {
        do {
            ...
            DebugConfig.debugPrint("❌ Error clearing all caches: \(error)")
        }
    }
